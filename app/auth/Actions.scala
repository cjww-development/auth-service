...
@Singleton
class Actions @Inject()(userActionWrapper: UserActionWrapper) extends AuthActions {
  ...
  class Unauthenticated() extends UnauthenticatedAction {
    ...
    private def unauthorised(body : OptionUserAction) = {
      userActionWrapper.withPotentialUser() {
        implicit account => body(account)
      }
    }
  }
...
trait AuthActions {
  def authorisedFor: AuthenticatedAction
