...
trait MockAuthMicroserviceConnector extends BeforeAndAfterEach with MockitoSugar with Fixtures {
  ...
  val mockAuthMicroserviceConnector = mock[AuthMicroserviceConnector]
  ...
  def mockGetIndividualUser(fetched: Boolean): OngoingStubbing[Future[CurrentUser]] = {
    when(mockAuthMicroserviceConnector.getUser(ArgumentMatchers.any())(ArgumentMatchers.any()))
      .thenReturn(if(fetched) Future(testCurrentUser) else Future.failed(new ForbiddenException("")))
  }
}
